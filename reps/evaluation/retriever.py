...
class DenseRetriever(BaseSearch):
    ...
    def search(self,
               ...
               top_k: int,
               score_function: str,
               **kwargs) -> Tuple[Dict[str, Dict[str, float]], List[float]]:
        """
        ...
        for query_itr in range(len(query_embeddings)):
            ...
            results: List[SearchResponse] = self.index.search(vector=embedding, limit=top_k)
    ...
    def encode_queries(self, queries: List[str], batch_size: int, **kwargs) -> List[List[float]]:
